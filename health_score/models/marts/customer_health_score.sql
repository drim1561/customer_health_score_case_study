...
component_scores as (
    ...
    select
        ...
        case
            when coalesce(m.recent_mau, 0) = 0 then 0
            when coalesce(m.prior_mau,  0) = 0 then 70   -- new/reactivated → neutral
            when (m.recent_submissions / nullif(m.recent_mau, 0)::float)
               / (m.prior_submissions  / nullif(m.prior_mau,  0)::float) >= 1.2  then 100
            when (m.recent_submissions / nullif(m.recent_mau, 0)::float)
               / (m.prior_submissions  / nullif(m.prior_mau,  0)::float) >= 1.0  then 80
            when (m.recent_submissions / nullif(m.recent_mau, 0)::float)
               / (m.prior_submissions  / nullif(m.prior_mau,  0)::float) >= 0.8  then 60
            when (m.recent_submissions / nullif(m.recent_mau, 0)::float)
               / (m.prior_submissions  / nullif(m.prior_mau,  0)::float) >= 0.6  then 40
            when (m.recent_submissions / nullif(m.recent_mau, 0)::float)
               / (m.prior_submissions  / nullif(m.prior_mau,  0)::float) >= 0.4  then 20
            else 0
        ...
        case
            when coalesce(m.recent_mau, 0) = 0 then 0
            when coalesce(m.prior_mau,  0) = 0 then 70
            when m.recent_mau / nullif(m.prior_mau, 0)::float >= 1.2 then 100
            when m.recent_mau / nullif(m.prior_mau, 0)::float >= 1.0 then 80
            when m.recent_mau / nullif(m.prior_mau, 0)::float >= 0.8 then 60
            when m.recent_mau / nullif(m.prior_mau, 0)::float >= 0.6 then 40
            when m.recent_mau / nullif(m.prior_mau, 0)::float >= 0.4 then 20
            else 0
        ...
        case
            when m.last_submission_date is null        then 0
            when m.days_since_last_submission <=   7   then 100
            when m.days_since_last_submission <=  30   then 80
            when m.days_since_last_submission <=  60   then 60
            when m.days_since_last_submission <=  90   then 40
            when m.days_since_last_submission <= 180   then 20
            else 0
        ...
        case
            ...
            when coalesce(m.products_active_recent, 0) / d.products_subscribed::float >= 1.00 then 100
            when coalesce(m.products_active_recent, 0) / d.products_subscribed::float >= 0.75 then 80
            when coalesce(m.products_active_recent, 0) / d.products_subscribed::float >= 0.50 then 60
            when coalesce(m.products_active_recent, 0) / d.products_subscribed::float >= 0.25 then 40
            when coalesce(m.products_active_recent, 0) > 0                                    then 20
            else 0
        end as product_adoption_score,

        -- Raw rate needed for Component 5
        case
            when coalesce(m.recent_mau, 0) = 0 then 0.0
            else m.recent_submissions / nullif(m.recent_mau, 0)::float
        end as recent_submissions_per_mau

    from dimensions d
    left join metrics    m on d.account_id = m.account_id
    left join benchmarks b on d.division   = b.division
                           and d.segment   = b.segment
...
with_relative_score as (
    ...
    select
        ...
        case
            when cs.recent_submissions_per_mau = 0                                                        then 0
            when b.median_submissions_per_mau  = 0                                                        then 80
            when cs.recent_submissions_per_mau / nullif(b.median_submissions_per_mau, 0) >= 1.5           then 100
            when cs.recent_submissions_per_mau / nullif(b.median_submissions_per_mau, 0) >= 1.0           then 80
            when cs.recent_submissions_per_mau / nullif(b.median_submissions_per_mau, 0) >= 0.75          then 60
            when cs.recent_submissions_per_mau / nullif(b.median_submissions_per_mau, 0) >= 0.50          then 40
            when cs.recent_submissions_per_mau / nullif(b.median_submissions_per_mau, 0) >= 0.25          then 20
            else 0
        end as relative_activity_score

    from component_scores cs
    left join benchmarks b on cs.division = b.division
                           and cs.segment = b.segment
